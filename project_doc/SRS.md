...
- **处理**:
    ...
    - *反爬应对*: 
        - 自动检测 461 状态码（Soft Ban），提示用户进行手动滑块验证。
        - 识别“请求成功但无数据”的异常响应，避免误判为爬取结束。
    - *特殊逻辑*: 识别并跳过“评论区”抓取。
- **输出**: JSON 对象
  ```json
...
- **使用限制**:
    ...
    - **反爬虫对抗**:
        - **User-Agent 一致性**: 必须确保爬虫使用的 User-Agent 与提取 Cookie 的浏览器环境一致，防止因指纹不匹配触发 461 验证。
        - **状态码监控**: 实时监控 461 等特殊状态码并提供人工介入提示。
