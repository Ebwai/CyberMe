...
本项目是一个面向个人知识管理与内容采集的 主要由2个需求组成：**多平台多模态高质量信息获取**和**多模态信息处理**，支持从以下平台的“收藏夹”或指定列表中自动拉取内容（包括文字，图片，视频【音频】等多模态信息）并将所有模态的信息转化为文字进行本地化存储（用文字表示所有模态的信息是为了后续的高性价比RAG建库和处理）
- Bilibili（B站）
- Douyin（抖音）
- Xiaohongshu（小红书）
